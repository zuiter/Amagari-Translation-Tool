...
Amagari Translation Tool 是一个面向 Minecraft 1.21.11 Fabric 的翻译辅助模组。它可以从地图目录、远程服务器/LAN 主机或 ParaTranz 项目直接加载语言 JSON 到客户端翻译表，不需要额外打包或启用资源包；同时提供双语切换和源文显示，方便地图与模组翻译校对。
...
- 书本中的可翻译文本会在后面附加 `ⓘ`，鼠标悬停 `ⓘ` 可在接近原版书页的浅色背景上查看源文；如果地图正文自带 hover/click 交互，正文会继续使用地图原提示与点击动作，不会与 ATT 源文框同时触发。
- 源文显示（包括告示牌源文 HUD）优先读取当前已启用的 Minecraft、资源包和模组自身的源语言资源，例如 `assets/*/lang/en_us.json`；找不到 key 时，才回退到本次会话已拉取的 ParaTranz 源语言条目。
- 物品 tooltip 和书本文本必须保留可翻译 key 才能反查源文。如果内容已经被模组或地图写成纯字面量文本，客户端无法自动恢复原始源文。语言 JSON 本身不包含颜色信息，因此物品源文只能继承原组件已有的颜色和排版样式；书本源文框会使用固定的书页风格来保证可读性。
