...
public class ParaTranzConfigScreen extends Screen {
	...
	public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
		...
		if (!status.isBlank()) {
			graphics.text(font, status, formLeft, maxCachedArtifacts.getY() + FIELD_HEIGHT + 4, status.equals(WorldLanguageMessages.paraConfigSaved(languageCode)) ? TEXT_COLOR : ERROR_COLOR);
		}
	}
