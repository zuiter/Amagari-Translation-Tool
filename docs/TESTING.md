...
- Run `/amagari_lang paratranz projects`; confirm it lists projects including `Permafrost-i18n`, and that clicking a project name runs the matching `/amagari_lang paratranz pull <project>` command.
...
- Run `/amagari_lang paratranz pull Permafrost-i18n` and confirm it downloads, caches, and applies the project translations.
...
- In a singleplayer integrated-server world, run `/amagari_lang paratranz config`, `/amagari_lang paratranz projects`, `/amagari_lang paratranz pull Permafrost-i18n`, and `/amagari_lang status`; confirm the config screen opens, chat feedback appears, and no red Brigadier parse error is shown.
...
- Press `H` with `sourceLanguage=en_us` and confirm vanilla/modded item helpers show the real English text from enabled Minecraft/resource-pack/mod language resources even when the pulled ParaTranz `en_us` export contains translated target text.
- After a successful ParaTranz pull, press `H` and confirm item tooltips can still fall back to source-language helper text from the pulled project when the key does not exist in enabled resource language files.
...
- With `H` enabled, open a written book containing translatable text from the pulled project and confirm a hoverable `ⓘ` marker appears after translated text; hovering the marker should show the source text.
