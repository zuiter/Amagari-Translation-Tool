...
- `build/libs/amagari-translation-tool-mc1.21.11-fabric-<version>.jar` exists.
...
- With an existing translatable text display and translatable sign loaded, trigger a normal resource reload and confirm both use the reloaded ATT language table without rejoining the world.
...
- While inside a world with menu background blur enabled, run `/amagari_lang paratranz config` from chat and confirm the first rendered frame does not crash with `Can only blur once per frame`; confirm the background is drawn once and all field labels remain visible above it.
...
- Run `/amagari_lang paratranz projects`; confirm it lists projects visible to the configured token, and that clicking a project name runs `/amagari_lang paratranz pull <project-id>` with that project's numeric ID.
...
- Type the first digits of a visible project ID after `/amagari_lang paratranz pull ` and confirm completion suggests the matching numeric project ID.
- Run `/amagari_lang paratranz pull <project-name-or-id>` once by name and once by numeric ID for a visible test project; confirm both forms select the same project and download, cache, and apply its translations.
...
- On Minecraft 1.21.11, leave a local world through the normal saving/progress screen and confirm pending resource-pack replacement and client language-state cleanup still complete.
...
- Run `/amagari_lang paratranz pull <missing-project-or-id>` and confirm it shows a clean not-found or ambiguous-project message with suggestions when applicable.
- In a singleplayer integrated-server world, run `/amagari_lang paratranz config`, `/amagari_lang paratranz projects`, `/amagari_lang paratranz pull <project-name-or-id>`, and `/amagari_lang status`; confirm the config screen opens, chat feedback appears, and no red Brigadier parse error is shown.
