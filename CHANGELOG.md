...
- 新增面向 Minecraft 1.21.10 Fabric 的适配分支：构建目标改为 Java 21，依赖更新为 Fabric API 0.138.4+1.21.10、Fabric Loader 0.19.2，并同步 1.21.10 的 Fabric 网络 payload、命令和客户端 mixin API。
- 远程语言数据改为分片发送：单个语言保留 4 MiB 压缩数据总上限，但每个网络 payload 最多发送 512 KiB，以适配 Minecraft 1.21.10 的普通自定义 payload 注册方式。
