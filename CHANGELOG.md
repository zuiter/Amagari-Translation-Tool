...
- 修复 Minecraft 1.21.11 通过进度界面退出世界时未进入 ATT 客户端清理流程的问题；现在会覆盖所有实际退出入口，确保待写入资源包、地图语言状态和 ParaTranz 会话状态正常收尾。
...
- 修复 1.21.11 客户端加载时 `GuiGraphicsMixin` 仍注入旧版 `renderTooltip(ItemStack)` 入口导致 Mixin 应用失败的问题。
...
- 新建面向 Minecraft 1.21.11 的 Fabric 模组骨架，模组名为 `Amagari Translation Tool`，mod id 为 `amagari_translation_tool`。
