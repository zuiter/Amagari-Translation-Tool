...
- Fix the 26.1 book-view mixin so entering a world no longer risks a network protocol disconnect when Minecraft loads `BookViewScreen`.
- Fix the Minecraft 26.1 sign-renderer mixin target so Fabric can apply client mixins during startup.
...
- 修复本地单人地图房主在未开启作弊时看不到或无法执行 `/amagari_lang push` 的问题；房主现在始终可以发布地图语言清单，专用/远程服务器上的普通玩家仍需 OP 权限。
- 优化 ParaTranz 设置界面的全尺寸与窄窗口布局：三个导出/地图写入选项现在会按实际文字宽度紧凑排列，空间足够时保持同一行，放不下时自动换行，不再把“写入当前地图资源包”固定在单独一行。
...
- 修复旧版 ParaTranz 配置缺少 `triggerExport` 字段时被错误读取为关闭的问题；迁移后现在会继续使用默认的开启状态。
- 修复资源包存在旧 `.att-pending` 时可能覆盖更新后的 `resources.zip`，以及新暂存写入失败时可能丢失旧恢复副本的问题；现在会优先保留时间更新的资源包，并在替换暂存文件失败时恢复旧副本。
...
- 新建面向 Minecraft 26.1 的 Fabric 模组骨架，模组名为 `Amagari Translation Tool`，mod id 为 `amagari_translation_tool`。
