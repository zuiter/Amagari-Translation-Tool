...
- 修复 1.21.8 客户端加载时 `GuiGraphicsMixin` 仍注入旧版 `renderTooltip(ItemStack)` 入口导致 Mixin 应用失败的问题。
...
- 新建面向 Minecraft 1.21.8 的 Fabric 模组骨架，模组名为 `Amagari Translation Tool`，mod id 为 `amagari_translation_tool`。
